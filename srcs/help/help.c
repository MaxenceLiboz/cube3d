...
/*   Updated: 2022/05/09 14:21:11 by tarchimb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_help_map(t_prg *prg)
{
		...
		<= prg->win.width / 4)
	{
		...
		{
			my_mlx_pixel_put(&prg->img, index.x, index.y,
				color_lerp(prg->draw.floor_color, prg->draw.sky_color,
					ft_inv_lerp_d(0, prg->win.height, index.y)));
			index.x++;
		}
		index.y++;
	}
